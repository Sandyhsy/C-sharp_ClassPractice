...
### Personal Homework 1（HW-1.zip）
   1. The BMI Calculator is an application that requests the user to enter their height and weight. It then proceeds to calculate the user's Body Mass Index (BMI) based on the provided values. Subsequently, the application determines whether the user falls into the categories of underweight, normal weight, or overweight.
   2. The Christmas Tree Generator is a program that asks the user to specify the size of the tree. The size entered by the user represents the height of each triangle that forms the tree. The program then constructs the tree using two triangles and adds a rectangular trunk at the bottom.
   3. The ID Validation Program is a utility that prompts the user to input their national identification number. After receiving the provided value, the program proceeds to evaluate the legitimacy of the entered ID. It determines whether the ID is valid or invalid based on predefined criteria.
